...
void triBestiaire(EspecePokemon bestiaire[]) {
    ...
    for (i = 0; i < 150; i++) {
        ...
        for (int j = i + 1; j < 150; j++) {
            if (strcmp(bestiaire[pos_min].nom, bestiaire[j].nom) > 0) {
                /*valeur positive si bestiaire[i].nom est après bestiaire[j].nom dans l’ordre alphabétique*/
                  best = bestiaire[j];
                  bestiaire[j] = bestiaire[pos_min];
                  bestiaire[pos_min] = best;
            }
        }
    }
}
...
void displayBestiaire(EspecePokemon bestiaire[]) {
    ...
    for (i = 0; i < 150; i++) {
        ...
        cout << " Bonbons pour evoluer : " << bestiaire[i].nbBonbonsPourEvoluer << endl;
        if (bestiaire[i].evolvesTo == nullptr)
            cout << "Pas d'evolution possible ! " << endl;
        else
            cout << "L'evolution est : " << bestiaire[i].evolvesTo << endl;
        cout << endl;
    }
}
